...
const RegisterAdmin = ({navigation}) => {
  ...
  const checkInput = () => {
    if (!name && !pass && !passConfirm) {
      ...
      setLoading(false);
      setErrorMessage(
        'Nama, Kata Sandi, dan Masukan Ulang Kata Sandi tidak boleh dikosongkan!',
      );
      ...
      fetch(`${HOST}/admin/register`, bodyData)
        .then(resJson => resJson.json())
        .then(res => {
          setLoading(false);
          if (res.status === 'success') {
            Alert.alert('Admin baru berhasil disimpan!');
    ...
    <View style={style.container}>
      ...
      <ScrollView style={style.scrollViewStyle}>
        <View style={style.formWrapper}>
          ...
          <Text style={style.label}>Pilih Tipe User</Text>
